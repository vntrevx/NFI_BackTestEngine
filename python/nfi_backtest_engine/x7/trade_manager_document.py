...
from .virtual_fees import compile_virtual_fee_policy

NFI_TRADE_MANAGER_IR_VERSION = "0.31.0"


def _adjustment_program_order(
    constants: dict[str, Any],
    program: dict[str, Any] | None = None,
) -> list[str]:
    if program is not None:
        return [action["tag"] for action in program["source_order"]]
    return [
...
def _frozen_constants(
    constants: dict[str, Any],
    *,
    source_system_compiled: bool = False,
) -> dict[str, Any]:
    ...
    if not isinstance(frozen["system_name_use"], str) or (
        frozen["system_name_use"] != frozen["system_v3_2_name"] and not source_system_compiled
    ):
...
def assemble_trade_manager_document(
    ...
    virtual_fees = compile_virtual_fee_policy(
        source.methods,
        constants,
        require_source_snapshot=compilation.system_exit_programs is not None,
    )
    if virtual_fees is not None and compilation.system_exit_programs is None:
        raise StrategyAnalysisError("virtual fees require source-compiled system exits")
    frozen = _frozen_constants(
        constants,
        source_system_compiled=(
            compilation.adjustment_dispatch is not None
            and compilation.system_exit_programs is not None
            and compilation.custom_exit_prefix is not None
        ),
    )
    family = "v4" if active_system_flags(constants)["is_system_v4"] else "v3"
    ...
    route_order = [
        name for name in compilation.managed_exit.long_route_order if name in supported_routes
    ]
    ...
    operation: dict[str, Any] = {
        ...
        "constants": frozen,
        "programs": {name: compilation.programs[name] for name in compilation.decision_roots},
    }
    if compilation.adjustment_dispatch is not None:
        operation["adjustment_dispatch"] = compilation.adjustment_dispatch
    if compilation.system_exit_programs is not None:
        operation["system_exit_programs"] = compilation.system_exit_programs
    if compilation.custom_exit_prefix is not None:
        operation["custom_exit_prefix"] = compilation.custom_exit_prefix
    if virtual_fees is not None:
        operation["virtual_fees"] = virtual_fees
    managed_tags = sorted({tag for route in managed_routes.values() for tag in route["entry_tags"]})
    ...
    if compilation.adjustment_constants is not None and compilation.adjustment_program is not None:
        operation["position_adjustment"] = {
            ...
            "system_version": frozen["system_name_use"],
            "source_callback": source.methods[f"long_grind_adjust_trade_position_{family}"].name,
            "decision_program": f"long_grind_entry_{family}",
            "program_order": _adjustment_program_order(
                compilation.adjustment_constants, compilation.adjustment_program
            ),
    ...
    if (
        ...
        operation["short_position_adjustment"] = {
            ...
            "system_version": frozen["system_name_use"],
            "source_callback": source.methods[f"short_grind_adjust_trade_position_{family}"].name,
            "decision_program": f"short_grind_entry_{family}",
            "program_order": _adjustment_program_order(
                compilation.short_adjustment_constants, compilation.short_adjustment_program
            ),
    ...
    if (
        ...
        operation["short_rebuy_adjustment"] = {
            ...
            "stateful_input_contract": compilation.short_rebuy_transition_program["input_contract"],
            "constants": compilation.rebuy_adjustment_constants,
    ...
    return {
        ...
        "implemented_steps": [
            ...
            *MANAGED_LONG_STATEFUL_STEPS,
            ...
            (
                "source-selected long/short system grind and rebuy position adjustment"
                if compilation.adjustment_dispatch is not None
                else "short system-v3.2 grind position adjustment"
            ),
            *(
                [
                    "source-selected common stops and cached profit targets",
                    "source custom exits before managed route dispatch",
                ]
                if compilation.system_exit_programs is not None
                else []
            ),
