...
Disabled callbacks may be absent; every open Native position still requires an
actual official observation at its timestamp and filled-order count.
...
def compare(native_path: Path, official_path: Path, output: Path) -> dict:
    ...
    for name in ("order_filled", "adjust_trade_position", "custom_exit"):
        for bucket in audit["callbacks"].get(name, {"outcomes": []})["outcomes"]:
