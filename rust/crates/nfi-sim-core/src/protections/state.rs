//! Runtime protection evaluation and pair-lock bookkeeping.
...
impl ProtectionState {
    pub(crate) fn locks(&self) -> &[PairLockState] {
        &self.locks
    }

    pub(crate) fn is_pair_locked(&self, pair: &str, timestamp_ms: i64, side: TradeSide) -> bool {
        let side = side_name(side);
        self.scope_is_locked(pair, timestamp_ms, side)
            || self.scope_is_locked("*", timestamp_ms, side)
    }

    pub(crate) fn after_trade_close(
        &mut self,
