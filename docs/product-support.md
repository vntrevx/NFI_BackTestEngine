...
- GitHub Releases is the current stable distribution. PyPI trusted publishing is a
  planned M29 gate and must not be advertised as available before it passes.
- Historical Spot and Futures certificates remain independently version-bound. The
  same-candidate combined status remains `preview` until M30 certification and the
  M31/M32 operating and clean-room gates pass.
